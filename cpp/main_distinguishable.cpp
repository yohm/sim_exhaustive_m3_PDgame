//
// Created by Yohsuke Murase on 2017/06/09.
//
...
int main(int argc, char** argv) {
  ...
  if( argc != 2 && argc != 3 ) {
    ...
    std::cerr << "  Usage : " << argv[0] << " <input_file_pattern> <output_file_pattern>" << std::endl;
  ...
  if( argc == 3 ) {
    ...
    if( str.IsDistinguishable() ) {
      std::cout << "distinguishable" << std::endl;
    }
    else {
      std::cout << "not distinguishable" << std::endl;
    }
  }
